...
def obtener_enlace_oferta(url):
    response = requests.get(url)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        # Busca la etiqueta <img> con la clase 'game_header_image_full'
        image_tag = soup.find('img', class_='game_header_image_full')
        if image_tag:
            image_src = image_tag['src']
            return image_src
        image_tag = soup.find('img', class_='package_header')
        if image_tag:
            image_src = image_tag['src']
            return image_src
    return None
def obtener_datos_juegos():
    ...
    URL = "https://store.steampowered.com/search/?specials=1" #URL del enlace de steam
    response = requests.get(URL)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        game_containers = soup.find_all('a', class_='search_result_row') #contenedor de los juegos de steam
        juegos_precios = [] #almacenamiento de los juegos guardados
        seen_games = set()
        count = 0
        #Imagen extraida de los juegos de steam y su direcion url usando las clases de estos
        for container in game_containers:
            ...
            if oferta_link:
                ...
                image_container = container.find('img', class_='game_header_image_full')
                if image_container:
                    image = image_container['src']
                else:
                    image = container.find('div', class_='search_capsule').find('img')['src']
            #precio de la oferta de los juegos
            ...
            #todo los juegos que se mostrara
            juegos_precios.append({
                "nombre": title, 
                "precio": price, 
                "imagen": image, 
                "url_oferta": detalles_link,
                "categorias": category, 
                "plataforma": ['steam']
            })
            seen_games.add((title, image, detalles_link))
            count += 1
            if count >= 10:
                break
...
def obtener_datos_gog():
    gog = 'https://www.gog.com/en/games?priceRange=0.24,29&discounted=true' #url de la pagian de gog
    response = requests.get(gog)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        game_containers = soup.find_all('a', class_='product-tile') #contenedor de los juegos de gog
        ...
        for container in game_containers:
            try:
                oferta_link = container['href'] #enlace de los jeugos
                image_container = container.find('store-picture', class_='ng-star-inserted')
                source_tag = image_container.find('source')
                image_url = source_tag['srcset'] if source_tag else "URL de imagen predeterminada" #imagen de los juegos extraida sacando su clase
                ...
                #precio de las ofertas sacando su clase
                price_element = container.find('span', class_='final-value ng-star-inserted')
                price_value = price_element.text.strip().replace('$', '') if price_element else "Precio no disponible"
                if price_value != "Precio no disponible":
                    price_value += ' €'
                ...
                seen_games.add((name, image_url, oferta_link))
                count += 1
                if count >= 10:
                    break
...
def obtener_juegos_sin_repetir(ofertas_steam, ofertas_gog):
    # Conjunto de juegos ya vistos en Steam
    juegos_vistos_steam = set((juego["nombre"], juego["imagen"]) for juego in ofertas_steam)
    
    # Filtrar juegos de GOG que no estén en Steam
    juegos_nuevos_gog = []
    for juego in ofertas_gog:
        if (juego["nombre"], juego["imagen"]) not in juegos_vistos_steam:
            juegos_nuevos_gog.append(juego)
            juegos_vistos_steam.add((juego["nombre"], juego["imagen"]))

    return juegos_nuevos_gog
        ...
        for container in game_containers:
            ...
            if oferta_link:
                ...
                if image_container:
                    ...
                    image_container = container.find('img', id='package_header_container')
                    if image_container:
                        image = image_container['src']
                    else:
                        image_container = container.find('div', class_='search_capsule').find('img')['src']
...
def obtener_datos_Aventura_gog():
    gog_aventura = 'https://www.gog.com/en/games?genres=adventure&priceRange=0.24,6&discounted=true'
    ...
    if response.status_code == 200:
        ...
        for container in game_containers:
            try:
                ...
                # Obtener el precio del juego y formatearlo correctamente
                price_element = container.find('span', class_='final-value ng-star-inserted')
                price_value = price_element.text.strip().replace('$', '') if price_element else "Precio no disponible"
                # Agregar el símbolo del euro a la derecha del precio
                if price_value != "Precio no disponible":
                    price_value += ' €'
            ...
            if oferta_link:
                ...
                if image_container:
                    ...
                    if image_container:
                        ...
                        image_container = container.find('div', class_='search_capsule')
                        if image_container:
                            image = image_container.find('img')['src']
                        else:
                            # Si no se encuentra ninguna imagen, usamos la del contenedor search_result
                            image = container.find('img', class_='search_result')['src']
...
def obtener_datos_Strategy_gog():
    gog_estrategia = 'https://www.gog.com/en/games?genres=strategy&priceRange=0.29,25&discounted=true'
...
def obtener_datos_Plataformer_gog():
    gog_plataforma = 'https://www.gog.com/en/games?tags=platformer&discounted=true'
...
def obtener_datos_Shooter():
    url_shooter = 'https://store.steampowered.com/search/?tags=1774&specials=1&ndl=1'
...
def obtener_datos_Simulation_gog():
    gog_shooter = 'https://www.gog.com/en/games?genres=simulation&priceRange=0.39,4&discounted=true'
    response = requests.get(gog_shooter)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        game_containers = soup.find_all('a', class_='product-tile')
...
def obtener_datos_Fight_gog():
    gog_fight = 'https://www.gog.com/en/games?tags=fighting&discounted=true'
...
def obtener_datos_Family():
    url_family = 'https://store.steampowered.com/search/?supportedlang=spanish&tags=5350&specials=1&ndl=1'
    response = requests.get(url_family)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        game_containers = soup.find_all('a', class_='search_result_row')
        juegos_precios = []
...
def obtener_datos_Sport_gog():
    gog_sport = 'https://www.gog.com/en/games?genres=sports&discounted=true'
    response = requests.get(gog_sport)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        game_containers = soup.find_all('a', class_='product-tile')
        ...
        return []
# Llamada a la función y muestra de los datos obtenidos
ofertas_sport_gog = obtener_datos_Sport_gog()
...
@app.route('/')
def lista_juegos():
    # Obtener datos de Steam y GOG
    ofertas_steam = obtener_datos_juegos()
    ofertas_gog = obtener_datos_gog()

    # Filtrar juegos de GOG que no estén en Steam
    nuevos_juegos_gog = obtener_juegos_sin_repetir(ofertas_steam, ofertas_gog)

    # Combinar todas las ofertas
    juegos_precios = ofertas_steam + nuevos_juegos_gog
...
def juegos_aventura():
    ...
    datos_epic = obtener_datos_Aventura() + obtener_datos_Aventura_gog()
...
def juegos_disparos():
    ...
    return render_template('shooter.html', juegos=juegos_precios)

@app.route('/simulacion')
def juegos_simulacion():
    juegos_precios = obtener_datos_Simulation() + obtener_datos_Simulation_gog()
    return render_template('simulacion.html', juegos=juegos_precios)
...
def juegos_puzle():
    ...
    return render_template('puzzle.html', juegos=juegos_precios)
